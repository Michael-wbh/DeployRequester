...
<head>
    ...
    <script type="text/javascript">
                        ...
                        if (resultData.data != null && resultData.data.length > 0) {
                            ...
                            if (lockedDeployRequestList != null && lockedDeployRequestList.length > 0) {
                                ...
                                for (var i = 0; i < lockedDeployRequestList.length; i++) {
                                    ...
                                    tableResultContent += "</tr>";
                                    unlockedDeployRequestStr +=","+onerow.projectcode + "-" + onerow.modulecode + "-" + onerow.moduletypecode;
                                }
                            }
                        }
                    }
         ...
         * @param moduleTypeId
         */
        function OnekeyUnlock() {
            if(unlockedDeployRequestStr == ""){
                alert("没有需要解锁的项目！");
                return;
            }else {
                unlockedDeployRequestStr = unlockedDeployRequestStr.substring(1);
                $.ajax({
                    type: "POST",
                    url: "<%=basePath%>depreq/OneKeyunlockDeployRequest",
                    data:unlockedDeployRequestStr,//json序列化
                    datatype:"json", //此处不能省略
                    contentType: "application/json; charset=utf-8",//此处不能省略
                    success:function(resultData){
                        if (resultData != null) {
...
<body>
    ...
    <table align="center" width="80%"  border="1" bordercolor="#a0c6e5" style="border-collapse:collapse;">
        <tr>
            <td align="center" bgcolor="#9acd32"><font id="processAction">以下是所有被锁定的模块</font>
                <input type="button" value="一键解锁" onclick="OnekeyUnlock();" style="float:right">
            </td>
        </tr>
        <tr>
            <td>
